...
namespace ConsoleApp.Combinatorics {
    /// <summary>
    ...
    public class Combinations<T> : IMetaCollection<T> {
        ...
        public class Enumerator : IEnumerator<IList<T>> {
            ...
            public Enumerator(Combinations<T> source) {
                myMyParent = source;
                myMyPermutationsEnumerator = (Permutations<bool>.Enumerator)myMyParent.myPermutations.GetEnumerator();
            }
            ...
            /// </summary>
            public void Reset() {
                myMyPermutationsEnumerator.Reset();
            }
            ...
            public bool MoveNext() {
                var ret = myMyPermutationsEnumerator.MoveNext();
                myMyCurrentList = null;
                return ret;
            }
            ...
            {
                if (myMyCurrentList != null) return;
                
                myMyCurrentList = new List<T>();
                var index = 0;
                IList<bool> currentPermutation = (IList<bool>)myMyPermutationsEnumerator.Current;
                foreach (var permutation in currentPermutation)
                {
                    if(permutation == false) {
                        myMyCurrentList.Add(myMyParent.myValues[index]);
                        if(myMyParent.Type == GenerateOption.WithoutRepetition) {
                            ++index;
                        }
                    }
                    else {
            ...
            /// </summary>
            private readonly Combinations<T> myMyParent;

            /// <summary>
            /// The current list of values, this is lazy evaluated by the Current property.
            /// </summary>
            private List<T>? myMyCurrentList;

            /// <summary>
            /// An enumertor of the parents list of lexicographic orderings.
            /// </summary>
            private readonly Permutations<bool>.Enumerator myMyPermutationsEnumerator;
        ...
        private void Initialize(IList<T> values, int lowerIndex, GenerateOption type) {
            ...
            {
                map.AddRange(myValues.Select((t, i) => i < myValues.Count - LowerIndex));
            }
            else {
            ...
            myPermutations = new Permutations<bool>(map);
        }
